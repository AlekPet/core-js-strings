...
function removeFirstOccurrences(str, value) {
  ...
  return startIndex !== -1
    ? str.substring(0, startIndex) + str.substring(endIndex)
    : str;
}
...
function sumOfCodes(str) {
  return str
    ? Array.from(str).reduce((sum, curValue) => sum + curValue.charCodeAt(), 0)
    : 0;
}
...
function formatTime(minutes, seconds) {
  const addZeros = (value) => {
    ...
    return absValue < 10
      ? valueStr.padStart(valueStr.length + 1, '0')
      : absValue;
  };

  const min = addZeros(minutes);
  const sec = addZeros(seconds);
  return `${min}:${sec}`;
}
...
function extractNameFromTemplate(value) {
  return value.slice(value.indexOf(', ') + 2, value.length - 1);
}
...
function encodeToRot13(str) {
  ...
  const alphavitLen = alphavit.length;
  ...
  for (let c = 0; c < arr.length; c += 1) {
    const char = arr[c];
    const charPos = alphavit.indexOf(char.toLowerCase());

    if (charPos > -1) {
      const plus13 = charPos + 13;
      const isUpperCase = char.toUpperCase() === char;

      if (plus13 >= alphavitLen) {
        const newChar = alphavit[Math.abs(alphavitLen - plus13)];
        text += isUpperCase ? newChar.toUpperCase() : newChar;
      } else {
        text += isUpperCase ? alphavit[plus13].toUpperCase() : alphavit[plus13];
      }
    } else {
      text += char;
    }
